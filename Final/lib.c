...
int count_user_borrowed_books(const char *username) {
    int count = 0;
    for (int i = 0; i < book_count; i++) {
        if (strcmp(books[i].borrowed_by, username) == 0) count++;
    }
    return count;
}
...
int return_book(const char *title, const char *username) {
    pthread_mutex_lock(&book_mutex);
    for (int i = 0; i < book_count; i++) {
        if (strcmp(books[i].title, title) == 0 && strcmp(books[i].borrowed_by, username) == 0) {
...
void generate_report() {
    ...
    for (int i = 0; i < book_count; i++) {
        if (strcmp(books[i].status, "Available") == 0) available++;
        else borrowed++;
    }
    printf("Available: %d\nBorrowed: %d\n", available, borrowed);
    ...
    for (int i = 0; i < book_count; i++) {
        if (strcmp(books[i].status, "Borrowed") == 0 && books[i].due_date < now) {
            printf("- %s (Borrowed by %s, Due: %s)",
                   books[i].title, books[i].borrowed_by, ctime(&books[i].due_date));
        }
    }
    pthread_mutex_unlock(&book_mutex);
}
